...
function CommandCard({ cmd }: { cmd: CommandInfo }) {
    ...
    const handleSave = useCallback(async () => {
        setSaving(true);
        const ok = await saveFile(apiBase, cmd.path, draft);
        setSaving(false);
        if (ok) {
            setContent(draft);
            setEditing(false);
    ...
    return (
        <div style={{
            ...
            <div className="flex items-center gap-2">
                ...
                {expanded && !editing && (
                    <button onClick={() => { setEditing(true); setDraft(content); }} title="Edit"
                        style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "rgba(255,255,255,0.55)", display: "flex" }}>
                        <Pencil size={13} />
                    </button>
                )}
                {editing && (
            ...
            {!expanded && cmd.description && (
                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", margin: "2px 0 0 24px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{cmd.description}</p>
            )}
            {expanded && !editing && (
                <pre style={{
            ...
            <p style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", margin: "4px 0 0", paddingLeft: 24, fontFamily: "monospace" }}>{cmd.path.replace(/.*\.claude\//, "~/.claude/")}</p>
        </div>
    );
}
    ...
    return (
        <div>
            <div className="flex items-center gap-3 mb-4 flex-wrap">
                ...
                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", marginLeft: "auto" }}>{filtered.length} command{filtered.length !== 1 ? "s" : ""}</span>
                <ViewModeToggle mode={viewMode} onMode={setViewMode} />
            </div>
            <FileViews
                mode={viewMode}
                accent={COLOR}
                items={filtered.map(c => ({ id: c.path, name: c.name, description: c.description, path: c.path, content: c.content, badge: c.source === "external" ? "custom" : undefined }))}
            />
        </div>
    );
}
