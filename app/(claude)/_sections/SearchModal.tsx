...
const GROUP_ORDER: (keyof SearchResults)[] = ["claudemd", "memory", "skills", "commands", "hooks", "sessions", "settings"];
...
export default function SearchModal() {
    ...
    const flatResults: Result[] = results
        ? GROUP_ORDER.flatMap(key => results[key] ?? [])
        : [];
    ...
    const doSearch = useCallback(async (q: string) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(apiBase(`/api/claude/search?q=${encodeURIComponent(q)}`));
    ...
    // Keep the active option scrolled into view for keyboard nav.
    useEffect(() => {
        resultsRef.current?.querySelector(`#search-opt-${activeIdx}`)?.scrollIntoView({ block: "nearest" });
    }, [activeIdx]);

    // Keyboard nav
    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (e.key === "Escape") { setOpen(false); return; }
        // Focus trap: Tab must not escape the palette to the page behind it.
    ...
    return (
        ...
        >
            ...
            <div
                role="dialog"
                aria-modal="true"
                aria-label="Search your Claude configuration"
                style={{
                ...
                <div style={{
                    ...
                    <input
                        ...
                        aria-activedescendant={hasResults ? `search-opt-${activeIdx}` : undefined}
                        aria-label="Search sessions, memory, skills, commands"
                    ...
                    <div style={{
                        ...
                        {query && (
                            <button onClick={() => setQuery("")} style={{
                                ...
                                color: "rgba(255,255,255,0.55)", display: "flex", padding: 2,
                            }}>
                                <X size={14} />
                            </button>
                        )}
                ...
                {/* Results */}
                <div ref={resultsRef} id="search-results-listbox" role="listbox" aria-label="Search results" style={{ maxHeight: 400, overflowY: "auto", padding: "4px 0" }}>
                    {loading && query.length >= 2 && (
                        <div style={{ padding: "20px 16px", color: "rgba(255,255,255,0.55)", fontSize: 13, textAlign: "center" }}>
                            Searching...
                        </div>
                    )}
                    ...
                    {!loading && query.length < 2 && (
                        <div style={{ padding: "20px 16px", color: "rgba(255,255,255,0.5)", fontSize: 12, textAlign: "center" }}>
