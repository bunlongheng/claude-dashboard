...
import {
    ...
    groupBy, calcCost, fmtNum, fmtCost, hexToRgba,
...
interface DailyStatsResponse { daily: DailyBucket[]; byModel: ModelBucket[]; tools: ToolBucket[]; totalTurns: number }
        ...
        <>
            ...
            {filteredDaily.length > 1 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                    ...
                    <div style={cardShell}>
                        ...
                        <p style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", marginBottom: 12 }}>
            ...
            {filtered.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                    ...
                    <div style={cardShell}>
                        <h3 style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.55)", marginBottom: 14 }}>Token Usage by Model</h3>
                        <DonutChart segments={modelDonutSegments} />
                    </div>
                    {/* Tokens by project */}
                    <div style={cardShell}>
                        <h3 style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.55)", marginBottom: 14 }}>Tokens by Project</h3>
                        <HBarChart items={byProject.slice(0, 8).map(p => ({ label: p.project, value: p.total }))} color="#4A9EFF" />
                    </div>
                    {/* Top tools */}
                    <div style={cardShell}>
                        <h3 style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.55)", marginBottom: 14 }}>Top Tools (by call count)</h3>
                        {topTools.length > 0 ? (
                        ...
                        {byProject.slice(0, 20).map((p, i) => {
                            ...
                            return (
                                <div key={p.project} className="grid grid-cols-[1fr_60px_80px_80px_70px] gap-1 px-3 py-1.5 border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors items-center">
                                    <span className="text-[10px] text-white/70 truncate flex items-center gap-1.5 uppercase"><AppIcon project={p.project} size={12} />{p.project}</span>
                    ...
                    {pageItems.map(t => {
                        ...
                        return (
                            <div key={t.session_id} className="grid grid-cols-[1fr_90px_70px_80px_100px_70px] gap-1 px-3 py-1.5 border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors items-center">
                                <span className="text-[10px] text-white/70 truncate flex items-center gap-1.5 uppercase"><AppIcon project={proj} size={12} />{proj}</span>
