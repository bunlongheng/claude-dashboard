...
import { folderToName } from "@/lib/project-utils";
...
export const GET = withErrorHandler(async () => {
    ...
    // Strict: only sessions with a live claude process attached (same set the
    // ActiveSessionsBanner uses). No mtime fallback - we want the two views
    // to always agree.
    ...
    for (const folder of fs.readdirSync(CLAUDE_DIR)) {
        ...
        // Use folderToName (knows "-Sites-" + keeps hyphenated basenames intact)
        // rather than the naive split-on-"-", which would turn
        // "-Users-hengb01-Sites-local-apps" into the name "apps" instead of "local-apps".
        const projectName = folderToName(folder);
        ...
        for (const file of fs.readdirSync(folderPath).filter(f => f.endsWith(".jsonl"))) {
            ...
            // Only sessions with a live claude process. Matches the banner.
