...
// Per-key (machine filter) response cache. The local branch walks every project
// folder and parses the first 12 KB of each .jsonl, so without this the dashboard's
// Promise.all re-scans ~1 GB on every poll. Live-session flags refresh inside TTL.
const CACHE_TTL_MS = 15_000;
const sessionsCache = new Map<string, { at: number; data: { projects: ProjectData[] } }>();
...
function getRemoteMachines(): RemoteMachine[] {
    ...
    if (!db) return [];
    try {
        return db.prepare("SELECT id, hostname, ip, port FROM machines").all() as RemoteMachine[];
    } catch {
        return [];
    } finally {
        db.close();
...
interface SessionData {
    ...
    createdAt: string; updatedAt: string; stale: boolean; live: boolean; machine?: string;
}
...
export const GET = withErrorHandler(async (req: Request) => {
    ...
    const cacheKey = machineFilter || "__local__";
    ...
    // Remote sessions (fetched only when an explicit machine is selected)
    ...
    sessionsCache.set(cacheKey, { at: nowMs, data: payload });
