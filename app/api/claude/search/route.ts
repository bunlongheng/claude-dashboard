...
function safeJson<T = Record<string, unknown>>(p: string): T | null {
...
// Content cache for the search corpus. This route fires per debounced keystroke;
// re-reading every memory/skill/command/CLAUDE.md file each time is the hot cost.
// stat is cheap, so re-read a file only when its mtime changes.
const contentCache = new Map<string, { mtime: number; content: string }>();
function cachedRead(p: string): string {
    try {
        const mtime = fs.statSync(p).mtimeMs;
        const hit = contentCache.get(p);
        if (hit && hit.mtime === mtime) return hit.content;
        const content = safeRead(p);
        contentCache.set(p, { mtime, content });
        return content;
    } catch {
        return safeRead(p);
    }
...
function searchMemory(q: string): Result[] {
    ...
    for (const folder of fs.readdirSync(PROJECTS_DIR)) {
        ...
        for (const file of fs.readdirSync(memDir).filter(f => f.endsWith(".md"))) {
            const content = cachedRead(path.join(memDir, file));
...
function searchClaudeMd(q: string): Result[] {
    ...
    if (fileExists(globalPath)) {
        const content = cachedRead(globalPath);
    ...
    if (dirExists(PROJECTS_DIR)) {
        for (const folder of fs.readdirSync(PROJECTS_DIR)) {
            ...
            const content = cachedRead(mdPath);
            const projectName = folder.replace(/-/g, "/").split("/").pop() ?? folder;
...
function searchCommands(q: string): Result[] {
    ...
    for (const base of dirs) {
        ...
        for (const plugin of fs.readdirSync(base)) {
            ...
            for (const file of fs.readdirSync(cmdsDir).filter(f => f.endsWith(".md"))) {
                ...
                const content = cachedRead(path.join(cmdsDir, file));
    ...
    if (dirExists(STANDALONE_CMDS_DIR)) {
        for (const file of fs.readdirSync(STANDALONE_CMDS_DIR).filter(f => f.endsWith(".md"))) {
            if (results.length >= LIMIT) break;
            const name = file.replace(".md", "");
            const content = cachedRead(path.join(STANDALONE_CMDS_DIR, file));
            if (matches(name, q) || matches(content, q)) {
                results.push({ title: `/${name}`, description: snippet(content), path: `/commands`, type: "commands" });
            }
        }
...
function searchHooks(q: string): Result[] {
    ...
    for (const base of dirs) {
        ...
        for (const plugin of fs.readdirSync(base)) {
            ...
            const raw = safeJson<HooksFile>(hookPath);
            if (!raw) continue;
...
export const GET = withErrorHandler(async (req: Request) => {
    // Results include snippets of CLAUDE.md / memory files and settings key
    // names - config-bearing content. Only same-site/bearer callers may search.
