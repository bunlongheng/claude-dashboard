...
// Response cache keyed by slim flag. This route is hit on every page load (sidebar
// nav fetches ?slim=1) plus the Overview, and it walks every project's CLAUDE.md /
// config / hooks / commands / memory files + a DB query. Config rarely changes, so
// a short TTL avoids re-scanning on each navigation.
const BRAIN_TTL_MS = 15_000;
// Payload shape varies between slim/full responses (slim strips content/body
// from files), and the cache only ever round-trips it via NextResponse.json -
// it's never read field-by-field, so `unknown` is the precise fit here.
const brainCache = new Map<string, { at: number; data: unknown }>();
...
export const GET = withErrorHandler(async (req: Request) => {
    // Full payload embeds settings.json / settings.local.json contents, hooks,
    // and memory bodies. Only same-site/bearer callers may read any of it.
    ...
    const hit = brainCache.get(cacheKey);
    if (hit && nowMs - hit.at < BRAIN_TTL_MS) {
    ...
    // Slim mode: strip content/body from files to reduce payload (~540KB -> ~20KB)
    const outProjects = slim
        ? projects.map(p => ({ ...p, files: p.files.map(({ content, body, ...rest }: ClaudeFile) => rest) }))
        : projects;

    const payload = {
        globalClaude: slim ? undefined : globalClaude,
