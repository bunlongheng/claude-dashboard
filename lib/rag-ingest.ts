...
export async function discoverSources(): Promise<SourceFile[]> {
  ...
  // 4. Obsidian vaults (configurable via OBSIDIAN_VAULTS, colon-separated)
  const vaultEnv = process.env.OBSIDIAN_VAULTS
    || [path.join(home, "Sites/notes"), path.join(home, "Documents/Obsidian Vault")].join(":");
  for (const vault of vaultEnv.split(":").map((v) => v.trim()).filter(Boolean)) {
    if (!fs.existsSync(vault)) continue;
    const notes = await glob("**/*.md", {
      cwd: vault,
      absolute: true,
      ignore: ["**/.obsidian/**", "**/.trash/**", "**/templates/**", "**/Templates/**"],
    });
    for (const p of notes) {
      sources.push({ path: p, type: "obsidian" });
    }
  }
